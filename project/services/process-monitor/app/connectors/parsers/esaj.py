"""Parser genérico 100% síncrono para HTML do e-SAJ.

Funciona para: TJSP, TJRS, TJMG, TJPE, e outros tribunais e-SAJ.
...
def detect_esaj_captcha_or_block(html: str) -> bool:
    markers = [
        ...
        "não sou robô", "verificação de segurança",
    ]
    lower = html.lower()
    return any(marker in lower for marker in markers)


def detect_esaj_error_page(html: str) -> str | None:
    lower = html.lower()
    if any(k in lower for k in ("não encontrado", "processo não localizado", "não foi localizado", "processo inexistente", "não existe processo")):
        return "not_found"
...
def _extract_field_flexible(html: str, label: str) -> str | None:
    ...
    match = re.search(
        rf"{re.escape(label)}[:\s]*</(?:th|td|dt|dd|label|span|strong)[^>]*>\s*<(?:td|dd|span|div|p)[^>]*>([^<]+)",
        html, re.IGNORECASE | re.DOTALL,
    )
    if match:
        return match.group(1).strip()
    # Padrão 2: label seguido de valor em texto puro
    match = re.search(
...
def parse_esaj_process_page(html: str, tribunal: str = "esaj") -> TribunalProcess:
    ...
    return TribunalProcess(
        ...
        data_distribuicao=_parse_date_br(
            _extract_field_flexible(html, "Distribuição")
            or _extract_field_flexible(html, "Data de Distribuição")
            or _extract_field_flexible(html, "Recebimento")
        ),
        orgao_julgador=_extract_field_flexible(html, "Órgão Julgador") or _extract_field_flexible(html, "Juiz") or _extract_field_flexible(html, "Relator"),
        vara=_extract_field_flexible(html, "Vara") or _extract_field_flexible(html, "Juízo") or _extract_field_flexible(html, "Foro"),
        comarca=_extract_field_flexible(html, "Comarca") or _extract_field_flexible(html, "Localidade") or _extract_field_flexible(html, "Foro"),
        status_raw=_extract_field_flexible(html, "Situação") or _extract_field_flexible(html, "Status"),
        valor_causa=_parse_money(
            _extract_field_flexible(html, "Valor da Ação") or _extract_field_flexible(html, "Valor da Causa") or _extract_field_flexible(html, "Valor")
        ),
        raw={"html_snippet": html[:2000]},
    )


def parse_esaj_movements(html: str) -> list[TribunalMovement]:
    movements: list[TribunalMovement] = []
    seen: set[str] = set()

    # Padrão e-SAJ: tabela com data + descrição
    pattern = re.compile(
    ...
    # Padrão alternativo: lista com data - descrição
    if not movements:
        pattern2 = re.compile(
            r"(\d{2}/\d{2}/\d{4})\s*[-–]\s*(.*?)(?:<br|<div|<p|</li>|$)",
            re.IGNORECASE | re.DOTALL,
        )
...
def _normalize_movement_type(desc: str) -> str:
    lower = desc.lower()
    if any(k in lower for k in ("sentença", "decisão", "deferimento", "indeferimento", "julgamento", "acórdão")):
        return "sentenca"
    if any(k in lower for k in ("intimação", "notificação", "citação", "penhora", "bloqueio", "mandado")):
        return "intimacao"
    if any(k in lower for k in ("audiência", "sessão", "julgamento")):
        return "audiencia"
    if any(k in lower for k in ("arquivamento", "baixa", "remessa", "baixa definitiva")):
        return "arquivamento"
    if any(k in lower for k in ("transito em julgado", "trânsito", "julgado")):
        return "transito_julgado"
    if any(k in lower for k in ("distribuição", "autuação", "recebimento", "recebimento pelo arquivo")):
        return "distribuicao"

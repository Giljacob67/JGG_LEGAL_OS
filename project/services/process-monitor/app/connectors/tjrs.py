...
logger = get_logger("connectors.tjrs")
rate_limiter = RateLimiter()


class TJRSConnector(TribunalConnector):
    @property
    ...
    def _mode(self) -> str:
        return getattr(settings, "TJRS_CONNECTOR_MODE", "fixtures")

    def _fixtures_dir(self) -> pathlib.Path:
        return pathlib.Path(getattr(settings, "TJRS_FIXTURES_DIR", "tests/fixtures/tjrs"))

    def _public_url(self) -> str | None:
        return getattr(settings, "TJRS_PUBLIC_SEARCH_URL", None) or None

    def _public_search_method(self) -> str:
        return getattr(settings, "TJRS_PUBLIC_SEARCH_METHOD", "GET").upper()

    def _public_search_headers(self) -> dict[str, str]:
        raw = getattr(settings, "TJRS_PUBLIC_SEARCH_HEADERS", None)
        if raw:
            try:
                return json.loads(raw)
            except json.JSONDecodeError:
                logger.warning("invalid_tjrs_headers_json", extra={"raw": raw})
        return {"User-Agent": "JGG-Legal-OS/1.0 (process-monitor)"}

    def _timeout(self) -> float:
        return getattr(settings, "TJRS_TIMEOUT_SECONDS", 20.0)

    def _rate_limit_seconds(self) -> float:
        return getattr(settings, "TJRS_RATE_LIMIT_SECONDS", 10.0)

    async def login(self, credentials: dict[str, Any]) -> bool:
        if credentials:
            raise ConnectorNotImplementedError("Login TJRS não implementado nesta fase", details={"error_code": "AUTH_NOT_IMPLEMENTED"})
        return True

    async def buscar_processo_por_numero(self, numero_cnj: str) -> ConnectorResult:
        if not self._is_enabled():
            return ConnectorResult(ok=True, tribunal=self.tribunal, source="tjrs_disabled", process=None, movements=[], documents=[], error_code="NOT_CONFIGURED", error_message="TJRS connector está desabilitado")
        with log_operation(logger, operation="buscar_processo_por_numero", tribunal=self.tribunal, connector="tjrs", numero_cnj=numero_cnj):
            mode = self._mode()
            if mode == "fixtures":
                return await self._buscar_fixtures(numero_cnj)
            if mode == "public_http":
                return await self._buscar_public_http(numero_cnj)
            return ConnectorResult(ok=True, tribunal=self.tribunal, source="tjrs", process=None, movements=[], documents=[], error_code="NOT_CONFIGURED", error_message=f"Modo TJRS não suportado: {mode}")
    ...
    async def _buscar_fixtures(self, numero_cnj: str) -> ConnectorResult:
        ...
        if not fixture_path.exists():
            return ConnectorResult(ok=True, tribunal=self.tribunal, source="tjrs_fixture", process=None, movements=[], documents=[], error_code="NOT_CONFIGURED", error_message="Fixture não encontrada")
    ...
    async def _buscar_public_http(self, numero_cnj: str) -> ConnectorResult:
        ...
        if not url:
            return ConnectorResult(ok=True, tribunal=self.tribunal, source="tjrs", process=None, movements=[], documents=[], error_code="NOT_CONFIGURED", error_message="TJRS_PUBLIC_SEARCH_URL não configurada")
        try:
            rate_limiter.wait_if_needed(self.tribunal)
        except ProcessMonitorError as exc:
            return ConnectorResult(ok=True, tribunal=self.tribunal, source="tjrs", process=None, movements=[], documents=[], error_code=exc.error_code, error_message=exc.message)
        try:
            ...
            return ConnectorResult(ok=True, tribunal=self.tribunal, source="tjrs", process=None, movements=[], documents=[], error_code="RATE_LIMITED", error_message=exc.message)
        except CaptchaOrBlockError as exc:
            rate_limiter.record_failure(self.tribunal, error_code="CAPTCHA_OR_BLOCK")
            return ConnectorResult(ok=True, tribunal=self.tribunal, source="tjrs", process=None, movements=[], documents=[], error_code="CAPTCHA_OR_BLOCK", error_message=exc.message)
    ...
    async def listar_andamentos(self, processo_id_tribunal: str) -> ConnectorResult:
        return ConnectorResult(ok=True, tribunal=self.tribunal, source="tjrs", process=None, movements=[], documents=[], error_code="NOT_IMPLEMENTED_SEPARATE", error_message="Andamentos são retornados na busca principal do TJRS")

    async def listar_documentos(self, processo_id_tribunal: str) -> ConnectorResult:
        return ConnectorResult(ok=True, tribunal=self.tribunal, source="tjrs", process=None, movements=[], documents=[], error_code="DOCUMENTS_NOT_IMPLEMENTED", error_message="Documentos não implementados no TJRS piloto")

    async def baixar_documento(self, documento_id_tribunal: str) -> bytes:
        raise ConnectorNotImplementedError("Download de documentos TJRS não implementado", details={"error_code": "DOCUMENT_DOWNLOAD_NOT_IMPLEMENTED"})

    async def healthcheck(self, live: bool = False) -> ConnectorHealth:
        if not self._is_enabled():
            return ConnectorHealth(tribunal=self.tribunal, connector="tjrs_pilot", status="disabled", details={"reason": "TJRS_CONNECTOR_ENABLED=false"})
        mode = self._mode()
        if mode == "fixtures":
            has_fixtures = (self._fixtures_dir() / "processo_publico_basico.html").exists()
            return ConnectorHealth(tribunal=self.tribunal, connector="tjrs_pilot", status="ok" if has_fixtures else "not_configured", details={"mode": "fixtures", "has_fixtures": has_fixtures})
        if mode == "public_http":
            url = self._public_url()
            if not url:
                return ConnectorHealth(tribunal=self.tribunal, connector="tjrs_pilot", status="not_configured", details={"mode": "public_http", "reason": "TJRS_PUBLIC_SEARCH_URL não configurada"})
            if not live:
                return ConnectorHealth(tribunal=self.tribunal, connector="tjrs_pilot", status="configured", details={"mode": "public_http", "url": url, "live_check": False})
            try:
                async with httpx.AsyncClient(timeout=self._timeout()) as client:
                    resp = await client.get(url)
                    resp.raise_for_status()
                    return ConnectorHealth(tribunal=self.tribunal, connector="tjrs_pilot", status="ok", details={"mode": "public_http", "url": url, "live_check": True})
            except Exception as exc:
                return ConnectorHealth(tribunal=self.tribunal, connector="tjrs_pilot", status="degraded", last_error_code=type(exc).__name__, last_error_message=str(exc), details={"mode": "public_http", "url": url, "live_check": True})
        return ConnectorHealth(tribunal=self.tribunal, connector="tjrs_pilot", status="not_configured", details={"mode": mode, "reason": "Modo não suportado"})

"""Conector TRF4 (eProc) — piloto seguro e desabilitado por padrão.
...
logger = get_logger("connectors.trf4")

rate_limiter = RateLimiter()


class TRF4Connector(TribunalConnector):
    """Conector piloto para TRF4 (eProc)."""
    ...
    @property
    def nome(self) -> str:
        return "TRF4 (eProc) — piloto"

    def _is_enabled(self) -> bool:
        return getattr(settings, "TRF4_CONNECTOR_ENABLED", False) is True

    def _mode(self) -> str:
        return getattr(settings, "TRF4_CONNECTOR_MODE", "fixtures")

    def _fixtures_dir(self) -> pathlib.Path:
        raw = getattr(settings, "TRF4_FIXTURES_DIR", "tests/fixtures/trf4")
        return pathlib.Path(raw)

    def _public_url(self) -> str | None:
        url = getattr(settings, "TRF4_PUBLIC_SEARCH_URL", None)
        return url if url else None

    def _public_search_method(self) -> str:
        return getattr(settings, "TRF4_PUBLIC_SEARCH_METHOD", "GET").upper()

    def _public_search_headers(self) -> dict[str, str]:
        raw = getattr(settings, "TRF4_PUBLIC_SEARCH_HEADERS", None)
        if raw:
            try:
                return json.loads(raw)
            except json.JSONDecodeError:
                logger.warning("invalid_trf4_headers_json", extra={"raw": raw})
        return {"User-Agent": "JGG-Legal-OS/1.0 (process-monitor)"}

    def _timeout(self) -> float:
        return getattr(settings, "TRF4_TIMEOUT_SECONDS", 20.0)

    def _rate_limit_seconds(self) -> float:
        return getattr(settings, "TRF4_RATE_LIMIT_SECONDS", 10.0)

    async def login(self, credentials: dict[str, Any]) -> bool:
        """Login não implementado nesta fase."""
        if credentials:
            raise ConnectorNotImplementedError(
                "Login TRF4 não implementado nesta fase",
    ...
    async def buscar_processo_por_numero(self, numero_cnj: str) -> ConnectorResult:
        if not self._is_enabled():
            return ConnectorResult(
                ...
                source="trf4_disabled",
                process=None,
                movements=[],
                documents=[],
                error_code="NOT_CONFIGURED",
                error_message="TRF4 connector está desabilitado (TRF4_CONNECTOR_ENABLED=false)",
            )
        ...
        with log_operation(
            ...
            return ConnectorResult(
                ...
                documents=[],
                error_code="NOT_CONFIGURED",
                error_message=f"Modo TRF4 não suportado: {mode}",
            )
    ...
    async def _buscar_fixtures(self, numero_cnj: str) -> ConnectorResult:
        ...
        if numero_cnj.replace("-", "").replace(".", "") not in html.replace("-", "").replace(".", ""):
            ...
            if not_found_path.exists():
                ...
                error = detect_trf4_error_page(not_found_html)
                if error == "not_found":
                    return ConnectorResult(
                        ok=True,
                        tribunal=self.tribunal,
    ...
    async def _buscar_public_http(self, numero_cnj: str) -> ConnectorResult:
        ...
        if not url:
            return ConnectorResult(
                ...
                documents=[],
                error_code="NOT_CONFIGURED",
                error_message="TRF4_PUBLIC_SEARCH_URL não configurada",
            )
        ...
        html = resp.text
        error = detect_trf4_error_page(html)
        if detect_trf4_captcha_or_block(html):
            ...
            return ConnectorResult(
                ...
                documents=[],
                error_code="CAPTCHA_OR_BLOCK",
                error_message="Captcha ou bloqueio detectado no TRF4",
            )
        if error == "not_found":
            ...
            return ConnectorResult(
                ...
                documents=[],
                error_code="NOT_FOUND",
                error_message="Processo não encontrado no TRF4",
            )
        if error == "system_error":
            ...
            return ConnectorResult(
                ...
                error_message="Erro no sistema do TRF4",
            )
    ...
    async def listar_andamentos(self, processo_id_tribunal: str) -> ConnectorResult:
        ...
        return ConnectorResult(
            ...
            error_message="Andamentos são retornados na busca principal do TRF4",
        )
    ...
    async def listar_documentos(self, processo_id_tribunal: str) -> ConnectorResult:
        return ConnectorResult(
            ...
            error_message="Documentos não implementados no TRF4 piloto",
        )

    async def baixar_documento(self, documento_id_tribunal: str) -> bytes:
        raise ConnectorNotImplementedError(
            "Download de documentos TRF4 não implementado",
    ...
    async def healthcheck(self, live: bool = False) -> ConnectorHealth:
        if not self._is_enabled():
            return ConnectorHealth(
                tribunal=self.tribunal,
                connector="trf4_pilot",
                status="disabled",
                details={"reason": "TRF4_CONNECTOR_ENABLED=false"},
            )
        ...
        if mode == "public_http":
            ...
            if not url:
                return ConnectorHealth(
                    ...
                    details={"mode": "public_http", "reason": "TRF4_PUBLIC_SEARCH_URL não configurada"},
                )
            if not live:
                return ConnectorHealth(
                    tribunal=self.tribunal,

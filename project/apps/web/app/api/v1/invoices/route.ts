...
import { faturaSchema, paginationSchema } from "@/lib/validations/zod-schemas";
...
export async function GET(req: NextRequest) {
  try {
    ...
    const pagination = paginationSchema.parse({
      ...
      sortBy: searchParams.get("sortBy") || "vencimento",
    ...
    return NextResponse.json({ data: faturas, meta: { page: pagination.page, limit: pagination.limit, total, totalPages: Math.ceil(total / pagination.limit) } });
  } catch (error) {
...
export async function POST(req: NextRequest) {
  try {
    ...
    const data = faturaSchema.parse(body);

    const fatura = await prisma.fatura.create({
      data: { ...data, emitidoPorId: user.id, createdAt: new Date(), updatedAt: new Date() },
    });
